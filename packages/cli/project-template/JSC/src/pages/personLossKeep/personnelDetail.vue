<template>
  <div class="yzd-page" v-wechat-title="title" ref="box">
    ...
    <dimension-box :dimensionData="lxchartTabList" :active="lxchartTabType" class="reset-scroll-elem" @selectHandle="
      (...args) =>
        selectHandle(
          'lxchartTabType',
          'setlxChartTabType',
...
export default {
  ...
  beforeRouteEnter(to, from, next) {
    if (from.path === '/personLossKeep/orglevel5' || from.path === '/personLossKeep/orglevel7') {
  ...
  methods: {
    ...
    rowClickFilter(item, prop) {
      ...
      if (item.ifHasNext == 1) {
        let path = `/personLossKeep/orglevel7`;
        let query = {
