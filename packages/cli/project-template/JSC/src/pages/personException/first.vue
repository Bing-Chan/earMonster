<template>
  <div class="yzd-page" v-wechat-title="title" ref="box">
    ...
    <dimension-box style="padding-top:0px" v-show="headerTabType != '-1'" :dimensionData="zyjmTypeList" :active="zyjmType"
      @selectHandle="
        (...args) =>
          selectHandle(
            'zyjmType',
    ...
    <dimension-box v-if="isZhiYing" :dimensionData="functionTypeList" :active="functionType" class="reset-scroll-elem"
      @selectHandle="
        (...args) =>
          selectHandle(
            'functionType',
    ...
    <!--pie chart-->
    <card-box v-if="headerTabType == '0' && functionType == 'line'" :title="pieChartTitle" :pageLoading="pieChartLoading"
      :hasData="pieHasData" class="reset-scroll-elem">
    ...
    <custom-table @headClick="headClick" @loadMore="loadMoreHandle" ref="customTable" :props="tablePropCollection"
      :topFixed="topFixed">
      <yto-search-panel v-show="searchPanlShow" :placeholder="placeholder" slot="searchPanel" :orgLevel="orgLevel"
        :getParams="getSearchParams" @select="handlerSelectOrgCode" :queryListAPI="queryListApi"></yto-search-panel>
  ...
  methods: {
    ...
    rowClickFilter(item, prop) {
      ...
      if (this.headerTabType != "-1") {
        ...
        let path = `/personException/secound`;
