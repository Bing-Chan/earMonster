<template>
    <mt-popup v-model="visible" :closeOnClickModal="closeOnClickModal" position="bottom" class="date-picker" @touchmove.native.stop.prevent>
        <mt-picker :slots="dateSlots" @change="onChange" :visible-item-count="visibleItemCount" class="mint-datetime-picker" ref="picker" show-toolbar rotate-effect>
            <div class="buttons">
                ...
                <span v-show="customDateError3" class="custom-error">最多选择15天</span>
            </div>
            <p v-if="dateType !== 'custom'" class="normal">{{currentValue | dateFormat(dateType)}}</p>

            <p v-if="dateType === 'custom' && !customDateType" class="custom">
                <span :class="['start',{'active': customActive === 'start', 'error': customDateError === 'start'}]" @click="customActive = 'start'">{{selectStart | dateFormat('date')}}</span>
                <span class="gap">至</span>
                <span :class="['end',{'active': customActive === 'end', 'error': customDateError === 'end'}]" @click="customActive = 'end'">{{selectEnd | dateFormat('date')}}</span>
            </p>
            <p v-if="dateType === 'custom' && customDateType==='customMonth'" class="custom">
                <span :class="['start',{'active': customActive === 'start', 'error': customDateError === 'start'}]" @click="customActive = 'start'">{{selectStart | dateFormat('month')}}</span>
                <span class="gap">至</span>
                <span :class="['end',{'active': customActive === 'end', 'error': customDateError === 'end'}]" @click="customActive = 'end'">{{selectEnd | dateFormat('month')}}</span>
            </p>
        </mt-picker>
    </mt-popup>
</template>
    ...
    export default {
        ...
        methods: {
            ...
            setSlotsByValues(values) {
                //["2009年", "01月", "01日"]
                /*
                if (this.pageType == 'orderTrack' && this.customstartDate !== '' && this.dateType === 'custom') {
                    this.$refs.picker.setValues(["2019年", "11月", "11日"]);
                } else {
        ...
        computed: {
            ...
            type() {
                if(this.dateType === 'custom' && this.customDateType === 'customMonth'){
                    return 'month'
                }else{
            ...
            ...mapState('common', [ 'customStartDay']),
        },
        watch: {
            value(val) {
            ...
            dateType(val) {
                this.generateSlots();
                // this.selectStart = new Date(this.customstartDate);
                if (val === 'custom') {
                    this.currentValue = this.customActive === 'start' ? this.selectStart : this.selectEnd
                }
            },
            customStartDay(){
                //let date = new Date(this.customStartDay);
               // let nyrNow = date.getFullYear() + "/" + (date.getMonth() + 1) + "/" + date.getDate();
                if (this.pageType == 'orderTrack' ) {
                   this.selectStart = new Date(this.customStartDay);
                   this.currentValue = this.customActive === 'start' ? this.selectStart : this.selectEnd
                }
            }
        },
        created() {
            ...
            let date = new Date();
            let nyrNow = date.getFullYear() + "/" + (date.getMonth() + 1) + "/" + date.getDate();
            let timestamp = new Date(nyrNow).getTime();
            this.selectStart = new Date(timestamp - 15000 * 3600 * 24);
            //this.selectStart = new Date(timestamp - 15000 * 3600 * 24);
            this.selectEnd = new Date(timestamp - 1000 * 3600 * 24);
        },
        mounted() {
