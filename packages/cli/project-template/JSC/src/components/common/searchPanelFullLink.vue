        ...
        methods: {
            ...
            querySelectOrg(keyword) {
                ...
                let params = {
                    ...
                    dimension: this.dimension ? this.dimension : this.orgType,     //查询维度:  ‘region’ 省区 ‘center’ 中心 ‘branch’ 分公司，'orgCenter' 直营中心，'customCenter' 非直营中心
                ...
                if (this.orgType === 'prov' || this.orgType === 'HEAD') {
                    params.dimension = 'region';
                }
                if (this.orgType !== 'prov' && this.linkScopeType === 'noSlotJoint') {
                    params.dimension = 'branch';
                }
                if (this.headerTabType === 'errorConstruction' && this.orgType !== 'prov' && this.buildOrgType === 'all') {
                    params.dimension = 'centerAndBranch';
                }
                if (this.headerTabType === 'errorConstruction' && this.orgType !== 'prov' && (this.buildOrgType === 'orgCenter' || this.buildOrgType === 'customCenter')) {
                    params.dimension = 'center';
                }
                if (this.headerTabType === 'errorConstruction' && this.orgType !== 'prov' && this.buildOrgType === 'branch') {
                    params.dimension = 'branch';
                }

                if (this.headerTabType === 'errorSend' &&  this.buildOrgType === '') {
                    params.dimension = 'centerAndBranch';
                }
