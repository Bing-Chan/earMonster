...
export default {
  ...
  methods: {
    ...
    selectHandle(type, funName, item, callback) {
      console.log(type, funName, item, callback)
    ...
    handlerSelectOrgCode(item, key) {
      ...
      this.queryDetailsList({
        ...
        ...{ keyWord: item.emplid ? item.name: item.orgName }
      });
    },
    queryTableReset() {
      this.searchType = 0;
    ...
    confirmDate(val) {
      if (this.activeType === "custom") {
        ...
        if (this.activeType == 'year') {
          this.setCurrentDate(new Date(dayjs(val).endOf('year')));
        } else {
    ...
    async queryDetailsList(params) {
      ...
      try {
        ...
        // debugger
        // localStorage.setItem('zanwu', res.status);
        if (
          !!res.data &&
          res.data.thead &&
          res.data.tbody &&
          ...
          if (isLoad || this.restTab) {
            ...
            if (info) {
              ...
              // this.sortType = "DESC";
              if(info.defaultSortType==""){
                this.sortType = "DESC";
              }else{
                this.sortType=info.defaultSortType.toUpperCase()
              }
            }
          }
    ...
    async queryLineChartData(params) {
      ...
      if (res.status === 0) {
        ...
        this.setEchartZoomOption(option, xdata, this.activeType == 'year' && dayjs().isSame(this.currentDate, 'year') && dayjs().month() + 1 <= 6 ? {
          startValue: 0,
          endValue: 6
        } : {});
        ...
        this.$nextTick(() => {
          ...
          !this.lineChartInstance
            ...
            : null;
          this.drawChart(this.lineChartInstance, option);
        });
      } else {
