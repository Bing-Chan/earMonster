  ...
  <div class="custom-table">
    ...
    <div class="yto-panel">
      ...
      <div class="list-custom" id="tableHead" ref="tableHead">
        <ul class="list-head" :class="{'tableFixed':tableHeadFixed}" :style="{top: props.topFixed + 'px','z-index': 100}">
          <template>
            <li class="expand" v-show="expand && category === 'centerTransfer'" style="flex-grow: 0"><div style="width: 25px;"></div></li>
            <li v-for="(item,index) in props.tableHeadData" :key="index" @click="headClick(item)" :style="{flex: item.prop === 'waybill_no' || item.prop === 'route_name' ? 1.4 : 1, 'padding-right': item.prop === 'time_num' ? '10px' : 0}"
                v-if="item.show">
              {{item.label}}
              <i
                v-if="item.sortable"
        ...
        <div class="infiniteScroll" v-infinite-scroll="loadMore" infinite-scroll-distance="10">
            <ul class="list-body" v-for="(item,index) in props.tableBodyData" :key="index"
              ...
              <slot name="list" :tableHeadData="props.tableHeadData" :item="item" :tableBodyData="props.tableBodyData" :index="index">
              </slot>
            </ul>
