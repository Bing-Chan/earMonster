...
import { addQuotalibrary, editQutotalLibrary, formulaConfig } from '/@/api/indicatorsLibrary/index.ts';

// 页面公用方法
export function usePages(state, getList) {
	const { proxy } = getCurrentInstance();
	// 公式完成
	const handleFormulaComplete = (item: any) => {
	...
	const handleScore = (val, callback) => {
		if ((state.dialogTitle == '修改指标' || state.dialogTitle == '指标公式') && val == 1) {
			ElMessageBox.confirm('更改得分来源将会清除已配置的公式，是否继续？', {
				confirmButtonText: '是',
				cancelButtonText: '否',
				type: 'warning',
			})
	...
	const handleFormulaType = (val) => {
		if (state.dialogTitle == '修改指标' || state.dialogTitle == '指标公式') {
			ElMessageBox.confirm('更改公式类型将会清除已配置的公式，是否继续？', {
				confirmButtonText: '是',
				cancelButtonText: '否',
				type: 'warning',
			})
	...
	const handleStep1Commit = async () => {
		proxy.$refs.indicatorRef.validate(async (valid: any) => {
			if (!valid) return false;
			state.commitLoading = true;

			if (state.addType == 1) {
				let res = await addQuotalibrary(state.indicatorForm);
				if (res.status == 0) {
				...
				let res = await editQutotalLibrary(state.indicatorForm);
				if (res.status == 0) {
					state.commitLoading = false;
					state.stepActiveIndex = 2;
	...
	const handleStep2Commit = async () => {
		proxy.$refs.indicatorRef1.validate(async (valid: any) => {
			if (!valid) return false;
			...
			state.commitLoading = true;
			let res = await formulaConfig(state.indicatorForm);
			if (res.status == 0) {
				state.commitLoading = false;
	...
	const reset = () => {
		if (state.indicatorForm.jobType == 1) {
			state.indicatorForm = {
				...
				quotaTypeId: 2,
				baseTargetState: 0,
				columnModifyFlag: '', // 指标结果被引用后是否能修改
	...
	const handleStep2Cancle = () => {
		if (state.addType == 1) {
			ElMessage.warning('指标公式未配置，请记得点击列表的【公式】进行配置');
		}
		state.dialogVisible = false;
