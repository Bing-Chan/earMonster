	...
	<div id="pages">
		...
		<div>
			<div v-if="activeName == 1">
				<caozuo ref="tab1Ref1"></caozuo>
			</div>
			<div v-else>
				<no-caozuo ref="tab1Ref2" ></no-caozuo>
			</div>
		</div>
	</div>
</template>
...
import caozuo from './src/caozuo.vue';
import noCaozuo from './src/noCaozuo.vue';
...
export default defineComponent({
	...
	setup() {
		...
		const handelTabs = async (tabs: any) => {
			state.activeName = tabs;
			await nextTick();
			proxy.$refs[`tab1Ref1${tabs}`].getList();
		};
