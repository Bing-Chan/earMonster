<template>
	<div id="pages">
		...
		<div class="content_list">
			...
			<div class="table_list">
				<el-table :data="tableData" stripe border style="width: 100%" height="100%">
					...
					<el-table-column fixed min-width="150" show-overflow-tooltip label="名称" prop="templateName" align="center">
						<template #default="scope">
							<el-button type="text" @click="addOrEdit(scope.row, true)">{{ scope.row.templateName }}</el-button>
						</template>
					</el-table-column>
					<el-table-column min-width="600px"  show-overflow-tooltip prop="monthSalaryStructure" label="月薪结构" align="center"></el-table-column>
					<el-table-column min-width="200px" prop="dailySalaryStructure" label="日薪结构" align="center"></el-table-column>
					<el-table-column min-width="200px"  prop="suitStaff" label="适用人员" align="center"></el-table-column>

					<el-table-column min-width="180px" prop="effectiveStartTime" label="有效期" align="center">
						<template #default="scope">
							{{ scope.row.effectiveStartTime + '~' + (scope.row.effectiveEndTime == null ? '' : scope.row.effectiveEndTime) }}
						</template>
					</el-table-column>
					<el-table-column prop="quoteStatus" min-width="100px" label="引用状态" align="center">
						<template #default="scope">
							{{ scope.row.quoteStatus ? '已引用' : '未引用' }}
						</template>
					</el-table-column>
		...
		<!--查看模板-->
		<el-dialog custom-class="step-dialog" :close-on-click-modal="false" title="查看薪资结构模板" v-model="viewFlag" center width="1000px">
			<div style="padding: 0px 15px">
				<el-tabs v-model="viewTab" class="demo-tabs">
					<el-tab-pane label="薪资结构" name="1">
						<yulan :dicList="dicList" :data="editForm" :selection="selection" :isEdit="false"></yulan>
					</el-tab-pane>
					<el-tab-pane label="结构明细" name="2">
