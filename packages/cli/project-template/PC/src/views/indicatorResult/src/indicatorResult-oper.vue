	...
	<div>
		<div class="query_box">
			<el-form :inline="true" label-position="left" :model="queryForm" ref="queryRef">
				...
				<el-form-item label="数据时间" prop="dateArr">
					<el-date-picker
						style="width: 100%"
				...
				<el-form-item label="考核对象" prop="dimensionName">
					<el-input v-model.trim="queryForm.dimensionName" clearable placeholder="请输入考核对象" style="width: 100%" maxlength="100"></el-input>
				</el-form-item>
				<el-form-item label="指标名称" prop="quotaName">
					<search
						@change="
							(item) => {
								queryForm.quotaName = item.label;
							}
						"
						placeholder="请输入指标名称"
						:control-type="SearchEnum.ZHIBIAOFORRES"
						isLoadAll
				...
				<el-form-item label="状态" prop="resultState">
					<el-select style="width: 100%" clearable v-model="queryForm.resultState" placeholder="请选择状态">
						<el-option key="1" label="启用" value="1" />
						<el-option key="0" label="禁用" value="0" />
					</el-select>
				</el-form-item>

				<!-- <el-form-item label="数据维度" prop="quotaDataDimensionType">
					<el-select style="width: 100%" v-model="queryForm.quotaDataDimensionType" placeholder="请选择" clearable>
						<el-option
				...
				<!-- <el-form-item label="更新人" prop="dataSource">
					<el-input style="width: 100%" v-model.trim="queryForm.dataSource" clearable placeholder="请输入更新人"></el-input>
				</el-form-item> -->
				<el-form-item>
		...
		<div class="content_list">
			...
			<div class="table_list table_fixed">
				<el-table @sort-change="handleSort" :data="tableData" stripe @selection-change="handleSelectionChange" border style="width: 100%">
					<template #empty><SysEmpty></SysEmpty></template>
					<el-table-column type="selection" fixed align="center" width="55"> </el-table-column>
					<!-- <el-table-column type="index" fixed label="序号" align="center" width="55"></el-table-column> -->
					<el-table-column prop="timeRangeTypeDesc" min-width="120" label="考核对象" align="center">
						<template #default="scope">
							{{ scope.row.dimensionCode + ' ' + scope.row.dimensionName.replace(scope.row.dimensionCode, '').replace('-', '') }}
						</template>
					</el-table-column>
					<el-table-column prop="usePosition"  min-width="120" show-overflow-tooltip label="工序岗位" align="center"></el-table-column>
					<el-table-column prop="shiftAttrVal" label="班次属性" width="100" align="center">
						<template #default="scope">
							{{ scope.row.shiftAttrVal ? scope.row.shiftAttrVal : '-' }}
						</template>
					</el-table-column>
					<el-table-column sortable prop="efficiencyValueStr" label="效率" min-width="100" align="center">
						<template #default="scope">
							<span class="td-number">{{ scope.row.efficiencyValueStr }}</span>
						</template>
					</el-table-column>
					<el-table-column sortable prop="priceValueStr" label="单价" min-width="100" align="center">
						<template #default="scope">
							<span class="td-number">{{ scope.row.priceValueStr }}</span>
						</template>
					</el-table-column>
					<el-table-column sortable prop="operateQuantityStr" label="操作量" min-width="110" align="center">
						<template #default="scope">
							<span class="td-number">{{ scope.row.operateQuantityStr }}</span>
						</template>
					</el-table-column>
					<el-table-column sortable prop="realityValStr" :class-name="'td-number'" label="绩效得分" width="120" align="center">
						<template #default="scope">
							<span class="td-number">{{ scope.row.realityValStr }}</span>
						</template>
					</el-table-column>
					<el-table-column prop="annTitle" label="数据时间" width="120" align="center">
						<template #default="scope">{{ getTimeStr(scope.row) }} </template>
					</el-table-column>
					<el-table-column prop="unitName" min-width="150" show-overflow-tooltip  label="单位名称" align="center">
						<template #default="scope">{{ scope.row.unitCode + ' ' + scope.row.unitName }} </template>
					</el-table-column>
					<el-table-column prop="quotaName" min-width="150" label="指标名称" align="center"></el-table-column>
					<el-table-column prop="resultStateDesc" label="状态" width="80" align="center"></el-table-column>
					<el-table-column prop="createTime" label="更新时间" width="160" align="center">
					...
					<el-table-column prop="efficiencyType" label="效率类型" min-width="150" align="center"></el-table-column>
					<el-table-column prop="efficiencyValueStr" label="效率" align="center">
						<template #default="scope">
							<span class="td-number">{{scope.row.efficiencyValueStr}}</span>
						</template>
					</el-table-column>
					<el-table-column prop="efficiencyUnit" label="数值单位" align="center"></el-table-column>

					<el-table-column prop="priceType" label="单价类型" align="center" min-width="150"></el-table-column>
					<el-table-column prop="priceValueStr" label="单价" align="center">
						<template #default="scope">
							<span class="td-number">{{scope.row.priceValueStr}}</span>
						</template>
					</el-table-column>
					<el-table-column prop="priceUnit" label="数值单位" align="center"></el-table-column>
					<el-table-column prop="operateQuantityType" label="操作量类型" min-width="150" align="center"></el-table-column>
					<el-table-column prop="operateQuantityStr" label="操作量" align="center">
						<template #default="scope">
							<span class="td-number">{{scope.row.operateQuantityStr}}</span>
						</template>
					</el-table-column>
					<el-table-column prop="operateQuantityUnit" label="数值单位" align="center"></el-table-column>
					<el-table-column prop="remark" label="备注" min-width="150" align="center"></el-table-column>
					<el-table-column prop="resultStateDesc" label="状态" width="80" align="center"></el-table-column>
					<el-table-column prop="createTime" label="更新人" width="100" align="center">
						<template #default="scope">{{ scope.row.updateUser }} </template>
					</el-table-column>
					<el-table-column prop="createTime" label="更新时间" width="160" align="center">
						<template #default="scope">{{ scope.row.updateTime }} </template>
					</el-table-column>
					<el-table-column label="操作" fixed="right" min-width="120" align="center">
						<template #default="scope">
							<el-button type="text" :disabled="(scope.row.updateUser || scope.row.createUser) == '系统对接'" @click="addOrEdit(scope.row)"
								>修改</el-button
							>
						</template>
					</el-table-column> -->
				...
				<el-col :xs="24" :sm="24" :md="24" :lg="24" :xl="24" class="mb20">
					<el-form ref="formRef" :model="editForm" :rules="formRules" label-position="right" label-width="150px">
						<div class="alert-title">
							<el-alert title="以下类型指标不支持新增指标数据：" type="warning" :closable="false" show-icon>
								<template #default>
									<div style="color: #333333">
										1.指标的【实际值来源】=“系统”，数据由系统对接，不支持人工添加 <br />
										2.所选指标的【得分来源】=“人工评分”，得分需走流程生成，不支持添加数据
									</div>
								</template>
							</el-alert>
						</div>
						<el-form-item label="指标名称" prop="quotaName">
							<search @change="handleZhibiaoChange" :control-type="SearchEnum.ZHIBIAOFORRES" v-model="editForm.quotaName"></search>
						</el-form-item>
						<el-form-item label="指标描述" prop="quotaDesc">
							<el-input disabled type="textarea" v-model.trim="editForm.quotaDesc" style="width: 100%" maxlength="100"></el-input>
						</el-form-item>
						...
						<el-form-item label="工序岗位" prop="usePosition">
							<el-select style="width: 100%" v-model="editForm.usePosition" clearable placeholder="请选择">
								<el-option v-for="(item, index) in editForm.usePositionList" :key="index" :label="`${item}`" :value="item" />
							</el-select>
						</el-form-item>
						<el-form-item label="班次属性" prop="shiftAttrCode" v-if="editForm.shiftFlag" required>
							<el-select v-model="editForm.shiftAttrCode" style="width: 100%" placeholder="请选择班次属性">
								<el-option
						...
						<el-form-item label="单位名称" prop="orgList">
							<org-tree
								:disabled="SearchEnum.PERSON == editForm.quotaDataDimensionId"
								ref="orgRef"
						...
						<el-form-item label="考核对象代码/名称" prop="dimensionName" v-if="editForm.quotaDataDimensionId == SearchEnum.COMPANY">
							<el-input v-model="editForm.dimensionName" disabled placeholder="" style="width: 100%" maxlength="100"></el-input>
						</el-form-item>
						<el-form-item label="考核对象代码/名称" prop="dimensionName" v-if="editForm.quotaDataDimensionId == SearchEnum.PERSON">
							<search
								ref="dataSearchRef"
								isDanwei
								style="width: 100%"
						...
						<el-form-item label="考核对象代码/名称" prop="dimensionName" v-if="editForm.quotaDataDimensionId == SearchEnum.DEPARTMENT">
							<el-select style="width: 100%" v-model="editForm.dimensionCode" @change="handlebumenChange">
...
<script lang="ts">
import { toRefs, reactive, defineComponent, onMounted, getCurrentInstance, nextTick, ref, toRaw, watchEffect } from 'vue';
import {
	indicatorResultAdd,
	indicatorResultList,
	indicatorResultUpdate,
	indicatorResultInfo,
	indicatorResultDisable,
	indicatorExport,
} from '/@/api/indicatorResult/index.ts';
	...
	setup(props) {
		...
		const state = reactive({
			...
			editForm: {
				...
				quotaDataDimensionId: 'Department',
				orgList: [],
				usePositionList: [],
			},
			formRules: {
				quotaName: [{ required: true, message: '请选择指标', trigger: 'change' }],
				...
				shiftAttrCode: [{ required: true, message: '请选择班次属性', trigger: 'change' }],
			},
			usePositionList: [],
		});
		...
		const handleUnitChange = (val: any) => {
			state.queryForm.unitName = val.label;
		};
		...
		const addOrEdit = async (row: any) => {
			...
			if (row) {
				state.formTitle = '修改指标数据';
				const res = await indicatorResultInfo({ id: row.id });
				if (res.status == 0) {
					...
					state.editForm.quotaDataDimensionId = toQuotaDataDimension(state.editForm.quotaDataDimensionType);
					if (state.editForm.quotaDataDimensionId == SearchEnum.DEPARTMENT) {
						reloadList(state.editForm.orgList[0].orgCode);
					}
					state.editForm.usePositionList = res.data.usePositions;
					// 转换数据维度
					// 转换指标
				} else {
		...
		const getList = async () => {
			...
			let res = await indicatorResultList(params);
		...
		const handleSelectedOrgTree = async (selectValue: any) => {
			...
			if (state.editForm.quotaDataDimensionId == SearchEnum.COMPANY) {
			...
			} else if (state.editForm.quotaDataDimensionId == SearchEnum.DEPARTMENT) {
				state.editForm.dimensionCode = '';
				state.editForm.dimensionName = '';
				state.editForm.dimensionLink = '';
			}
		...
		const handleSave = () => {
			proxy.$refs.formRef.validate(async (valid: any) => {
				...
				if (state.editForm.id && state.editForm.id > 0) {
					res = await indicatorResultUpdate(toRaw(state.editForm));
				} else {
					res = await indicatorResultAdd(
						toRaw({
							...state.editForm,
						})
					);
		...
		const handleZhibiaoChange = async (item: any) => {
			state.editForm.quotaId = item.value;
			state.editForm.quotaName = item.label;
			state.editForm.quotaDesc = item.desc;
			state.editForm.shiftFlag = item.shiftFlag;
			state.editForm.quotaDataDimensionId = toQuotaDataDimension(item.item.quotaDataDimensionId);
			state.editForm.dimensionName = '';
			state.editForm.dimensionCode = '';
			state.editForm.dimensionLink = '';
			state.editForm.usePositionList = item.item.usePositions;
			state.editForm.usePosition = '';
			state.editForm.orgList = [];
			...
			if (state.editForm.orgList.length > 0 || state.editForm.quotaDataDimensionId == SearchEnum.PERSON) {
				await nextTick();
				let code = state.editForm.orgList.length > 0 ? state.editForm.orgList[0].orgCode : '';
				proxy.$refs.dataSearchRef.setControlType(state.editForm.quotaDataDimensionId, code);
			}
		};
		...
		const handleWeiDuChange = (item: any) => {
			...
			if (state.editForm.quotaDataDimensionId == SearchEnum.PERSON) {
		...
		const isDisabledDanwei = () => {
			return state.editForm.quotaDataDimensionId;
		};
		...
		const handleExport = async () => {
			...
			if (state.queryForm.dateArr && state.queryForm.dateArr.length > 0 && state.queryForm.dateArr[0] == state.queryForm.dateArr[1]) {
				state.queryForm.timeRangeStart = state.queryForm.dateArr[0];
				state.queryForm.timeRangeEnd = state.queryForm.dateArr[1];
			} else {
				ElMessage.error('请将‘数据时间范围’选择到某一天，再点击‘导出’');
				return false;
			}
			...
			let fileName = '指标数据' + dayjs(new Date()).format('YYYY-MM-DD') + '.xls';
				...
				timer = setTimeout(async () => {
					...
					if (res.status == 0) {
						state.options = res.data.map((x) => {
							return { value: x.id, label: x.quotaName };
						});
					}
					loading.value = false;
				}, 500);
		...
		const handleSort = async (column) => {
			state.currentPage = 1;
			if (column.order) {
				// 排序字段 效率：efficiencyValue
				// 单价：priceValue
				// 操作量：operateQuantity
				// 实际值： realityVal
				// switch (column.prop) {
				// 	case 'operateQuantityStr':
				// 		state.queryForm.column = '';
				// 		break;
				// 	case 'priceValueStr':
				// 		state.queryForm.column = '单价：priceValue';
				// 		break;
				// 	case 'efficiencyValueStr':
				// 		state.queryForm.column = 'efficiencyValue';
				// 		break;
				// 	case 'realityValStr':
				// 		state.queryForm.column = column.prop;
				// 		break;
				// }
				state.queryForm.column = column.prop.replace('Str', '');
				state.queryForm.sort = column.order ? column.order.replace('ending', '') : '';
			} else {
				state.queryForm.column = '';
