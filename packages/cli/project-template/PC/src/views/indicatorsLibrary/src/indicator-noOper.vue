		...
		<div class="content_list" v-loading="loading">
			...
			<div class="table_list">
				<el-table :data="tableData" stripe @selection-change="handleSelectionChange" border style="width: 100%">
					...
					<el-table-column prop="name" label="是否有目标值" align="center" width="120">
						<template #default="scope">
							{{ scope.row.baseTargetState ? '是' : '否' }}
						</template>
					</el-table-column>
			...
			<div class="step-content" v-if="stepActiveIndex == 1">
				<el-form label-position="right" label-width="125px" ref="indicatorRef" :model="indicatorForm" :rules="formRules">
					...
					<el-form-item prop="baseTargetState" required>
						...
						<el-radio-group v-model="indicatorForm.baseTargetState" :disabled="indicatorForm.modifyFlag == 'no' || indicatorForm.formulaModifyFlag">
					...
					<el-form-item label="指标分类" prop="quotaCategoryId" required>
						<el-select
							style="width: 100%"
							:disabled="indicatorForm.modifyFlag == 'no'"
							v-model="indicatorForm.quotaCategoryId"
							clearable
							placeholder="请选择"
							@change="handleChangeQuotaCate"
						>
							<el-option
								v-for="(item, index) in dictionaryList['sys_type']"
								:key="index"
			...
			<div class="step-content" v-if="stepActiveIndex == 2">
				<el-form label-position="right" label-width="125px" ref="indicatorRef1" :model="indicatorForm" :rules="formRules">
					<el-form-item prop="scoreValueSource" required>
						...
						<el-radio-group @change="handleScore" v-model="indicatorForm.scoreValueSource" :disabled="scoreDisable">
					...
					<el-form-item label="公式类型" prop="formulaType" required v-if="indicatorForm.scoreValueSource == 2">
						<el-radio-group @change="handleFormulaType" v-model="indicatorForm.formulaType">
							<el-radio :label="1">通用公式</el-radio>
							<el-radio :label="2">自定义对象公式</el-radio>
						</el-radio-group>
					</el-form-item>
					<el-form-item :label="' '" prop="commonCalculationFormula" v-if="indicatorForm.scoreValueSource == 2 && indicatorForm.formulaType == 1">
						<formula
							...
							:jobType="2"
							:parameType="2"
							ref="commonCalculationFormulaRef"
						></formula>
					</el-form-item>
					<el-form-item label-width="0" v-if="indicatorForm.scoreValueSource == 2 && indicatorForm.formulaType == 2">
						<div class="formla-form">
							<div class="formla-list">
								<custom-formla
									:quotaDataDimensionId="indicatorForm.quotaDataDimensionId"
									:dictionaryList="dictionaryList"
									:table-list="indicatorForm.quotaFormulaConfigReqList"
									ref="formlaRef"
								></custom-formla>
							</div>
						</div>
					...
					<el-form-item prop="actualValueSource" required v-if="indicatorForm.scoreValueSource == 2">
						...
						<el-select style="width: 100%" v-model="indicatorForm.actualValueSource" clearable placeholder="请选择">
...
<script lang="ts">
import { defineComponent, getCurrentInstance, reactive, toRefs, onMounted, nextTick, ref } from 'vue';
import {
	getIndicatorsList,
	addQuotalibrary,
	editQutotalLibrary,
...
import CustomFormla from '/@/components/CustomFormla.vue';
...
export default defineComponent({
	...
	setup() {
		...
		const usePageRefs = usePages(state, getList);
		...
		const getDictionaryList = async () => {
			let params = ['sys_type', 'sys_data', 'sys_unit', 'sys_applicable'];
		...
		const addorEditIndicator = async (row: any, stepIndex: any) => {
			...
			if (row) {
				...
				if (res.status == 0) {
					nextTick(() => {
						...
						state.indicatorForm.baseTargetState = Number(state.indicatorForm.baseTargetState);
						if (state.indicatorForm.quotaCategoryId == 'sys_type_veto') {
							state.indicatorForm.scoreValueSource = 1
							state.scoreDisable = true
						} else {
							state.scoreDisable = false
						}
					});
				} else {
			...
			if (proxy.$refs.commonCalculationFormulaRef) {
				proxy.$refs.commonCalculationFormulaRef.reset();
			}
		};
		...
		const handleChangeQuotaCate = (quotaCategoryId: any) => {
			console.log(quotaCategoryId)
			if(quotaCategoryId == 'sys_type_veto'){
				state.indicatorForm.scoreValueSource = 1
				state.scoreDisable = true
			}else{
				state.scoreDisable = false
