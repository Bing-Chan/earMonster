	...
	<div>
		...
		<div class="content_list">
			...
			<div class="table_list">
				<el-table :data="tableData" stripe border style="width: 100%">
					...
					<el-table-column prop="paramName" label="参数名称" min-width="100" align="center"></el-table-column>
					<el-table-column prop="shiftFlag" label="班次属性" min-width="50" align="center">
						<template #default="scope">
							{{ scope.row.shiftFlag ? '有' : '无' }}
						</template>
					</el-table-column>
					<el-table-column prop="usePosition" label="适用工序岗位" min-width="150" align="center" show-overflow-tooltip></el-table-column>
					...
					<el-table-column prop="paramDataDimensionValue" label="参数数据维度" min-width="100" align="center"></el-table-column>
					<el-table-column prop="paramProperties" label="参数属性" min-width="50" align="center">
						<template #default="scope">
							{{ scope.row.paramProperties == 1 ? '系统对接' : '人工维护' }}
						</template>
					</el-table-column>
					<el-table-column prop="createUser" label="更新人" width="100" align="center">
						<template #default="scope">
		...
		<el-dialog :close-on-click-modal="false" :title="dialogTitle" v-model="dialogVisible" width="600px" @close="handleCancle">
			<el-form label-position="right" label-width="125px" ref="parameterRef" :model="parameterForm" :rules="formRules">
				...
				<el-form-item label="适用工序岗位" prop="usePositionList">
					<el-select :disabled="parameterForm.dimensionFlag" multiple style="width: 100%" v-model="parameterForm.usePositionList" clearable placeholder="请选择（支持多选）">
				...
				<el-form-item label="有无班次属性" prop="shiftFlag" required>
					<el-radio-group v-model="parameterForm.shiftFlag" :disabled="parameterForm.dimensionFlag">
	...
	setup() {
		...
		const state = reactive({
			...
			formRules: {
				...
				usePositionList: [{ required: true, message: '请选择适用工序岗位', trigger: ['blur', 'change'] }],
			},
			zhibiaoFlag: false,
			selectListString: '',
		});
		...
		const addorEditParameter = (row: any) => {
			if (row) {
				...
				row.shiftFlag = Number(row.shiftFlag);
				nextTick(() => {
					state.parameterForm = { ...row };
					state.parameterForm.usePositionList = row.usePosition == "" ? [] : row.usePosition.split('、')
				});
