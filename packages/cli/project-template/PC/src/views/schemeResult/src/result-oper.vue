		...
		<div class="content_list">
			...
			<div class="table_list">
				<el-table :data="tableData" @selection-change="handleSelectionChange" stripe border style="width: 100%">
					...
					<el-table-column prop="unitName" label="单位名称" align="center" min-width="120px"></el-table-column>
					<el-table-column prop="deptName" label="部门名称" align="center" min-width="100px"></el-table-column>
					<el-table-column prop="jobName" label="岗位名称" align="center" min-width="160px"></el-table-column>
					...
					<el-table-column prop="totalScoreStr" label="考核结果" align="center">
						<template #default="scope">
							{{ scope.row.totalScoreStr != null ? scope.row.totalScoreStr : '-' }}
						</template>
					</el-table-column>
					<el-table-column prop="assessDateShow" label="考核时间" align="center" min-width="100px"></el-table-column>
					<el-table-column prop="assessCycleTypeValue" label="考核周期" align="center"></el-table-column>
					<el-table-column prop="schemeName" label="考核表名称" align="center" min-width="180px"></el-table-column>
	...
	setup(props, { emit }) {
		...
		const state = reactive({
			queryForm: {
				schemeRecordId: 0,
				schemeName: '',
				assessCycleType: 'sys_period_day',
				lx: '',
				appraiseState: '',
				calculateMode: '',
				userCode: '',
		...
		const bandReset = () => {
			nextTick(() => {
				...
				if (state.formSchame) {
					...
					state.queryForm.value1 = [new Date().getTime() - 3600 * 1000 * 24, new Date().getTime() - 3600 * 1000 * 24];
				}
			});
		};
		...
		onMounted(() => {
			...
			initSystemTime((time) => {
				...
				if (!state.formSchame) {
					state.queryForm.assessStartDate = defalutTime;
					state.queryForm.assessEndDate = defalutTime;
					state.queryForm.value1 = [defalutTime, defalutTime];
				} else {
					emit('formSchame');
				}
				if (state.formSchame) {
